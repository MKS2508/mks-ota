...
modelo: mks-ota-roadmap (authority=true) · 1 outcomes · 4 tracks · 0 spikes · 13 milestones
...
milestones:
  ...
  ▸ track/partial-channel/el-suite-depende-de-un-fixture-remoto-mutable  ○ queued — CODIGO_VIVO con medicion - 9 de 54 tests de 'cargo test --lib' estan ROJOS y no es codigo nuestro: download::tests (4), verify::tests (3) e install::full::macos::tests (2) comparten un fixture que se DESCARGA EN RUNTIME de una URL pineada, y el hub responde 404. Medido: curl a /api/components/wraith-linux/download/0.1.0/darwin/aarch64/Wraith.app.tar.gz -> http=404, size=50; el 'artefacto' que aterriza en el tmpdir son los 50 bytes del cuerpo del error ({"error":"File not found"}), de ahi el 'invalid gzip header'. NO es bloqueo de red: el curl sale rc=0 y el hub contesta. Ese artefacto simplemente ya no esta publicado (encaja con el DROP TABLE releases del M6 del hub, 28-ago). Control positivo del instrumento: el mismo cargo test --lib filtrado a install::partial da rc=0, o sea el runner sabe dar verde. || POR QUE ES DEUDA Y NO ANECDOTA: un suite que depende de un artefacto remoto MUTABLE se pone rojo cuando alguien de otro repo borra algo, y su rojo no dice nada sobre este codigo. Peor: normaliza el rojo, y un suite cronicamente rojo deja de discriminar — nadie mira los 9 y el dia que sean 10 tampoco. Opciones: fixture local commiteado (bytes en tests/fixtures, que ya es el patron del canal parcial), o el test se salta explicito con motivo cuando el fetch falla, NUNCA un ignore mudo. || Verificado como PREEXISTENTE, no regresion: la lista de nombres de los 9 fallos es identica antes y despues de la lane staged-invalidation (diff de las dos listas, rc=0).
  ▸ track/partial-channel/rollback-monta-previous-sin-ninguna-puerta  ○ queued — TERCERA PUERTA SIN CERRAR, escalada por el ejecutor de staged-invalidation en vez de decidirla el solo - rollback() (apply.rs:195-207) monta el slot "previous" y le pone verified=true SIN ningun gate, y ese slot "previous" no registra en ningun sitio contra que nativo se construyo. Las dos puertas que cierra la lane (invalidate en el arranque, activate_staged al montar) NO cubren esta. El estado que la hace alcanzable es exactamente el especimen de waxin: previous poblado, active null. MITIGA SOLO: se autocura al siguiente arranque porque native_version_at_swap no se restaura, asi que la ventana no es permanente. NO SE TOCO a proposito - el contrato definia dos puertas y ampliar scope por cuenta propia es rediseñar. El arreglo natural es darle a "previous" su propio sello, mismo patron que native_version_at_stage.
